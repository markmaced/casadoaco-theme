...
function filtrar_calculadoras_callback()
{
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            $query->the_post();
            ?>
            <div id="<?php echo get_post_field('post_name', get_the_ID()); ?>" class="bg-white rounded shadow flex flex-col items-center p-5 cursor-pointer text-center formato-btn" data-option-id="<?php echo get_the_ID()?>">
